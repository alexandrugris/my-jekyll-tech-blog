...
If end up with the principal `F` factor explaining less of the variance any of the `X` initial factors, PCA should be applied to standardized random variables. That is, from which the mean was substracted and then we divided them by their standard deviation. For our `A` matrix, the matrix of all `X` factors, the function is as follows:
