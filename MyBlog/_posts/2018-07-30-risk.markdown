...
Now that we have a basic mathematical understanding on how to simulate probabilities, the remaining question is how to draw the input data, the expected goals. Here is a simple additive model:
...
if __name__ == "__main__":
    ...
    fig, ax = plt.subplots(len(matches), 5)
    for i in range(0, len(matches)):
        for j in range(0, 5): # 5 different runs to show a nice chart                
            p = ax[i][j]
            simulate(p, matches[i], 1000)
